...
var connectionUrl="185.45.23.126:8888/ws";
var hueInstance;
...
var w=new Arip("LamppostScheduler","scheduler",connectionUrl);
    ...
    w.registerFunction('giveMeXMinutesOfLight','[{"name":"lightId","type":"string"},{"name":"mins","type":"int"}]', '');
    ...
    createSuggestion = function(lampId,date,dimmLevel){
        console.log("LamppostScheduler - "+lampId+", DimmLevel: "+dimmLevel);
    ...
    //PUBLIC - turn on lamppost for x minutes, and then go back to previous luminance
    giveMeXMinutesOfLight = function(lightId, mins){
        if(lamps[lightId].state.hasOwnProperty("brightness")){
            var brightness=lamps[lightId].state.brightness;
            setLightState(lightId, 254);
            var date = new Date();
            date.setMinutes(date.getMinutes()+mins)
              
            schedule.scheduleJob(date, (function(){
                console.log("SCHEDULE EXECUTED: "+brightness);
                if(lamps[lightId].state.brightness >= 254){
                    setLightState(lightId, brightness);
                }
            }).bind(null, lightId, brightness));
        }
    }
...
function setLightState(lightId, strength){
    console.log("setLightState function 1: "+strength);

    var tempState = lightState.create();
    
    if(strength>0)
        tempState.on().bri(strength);
    else
        tempState.off().bri(strength);
        
    console.log("setLightState function 2: "+JSON.stringify(tempState));
...
function searchForNewLights(){
    ...
    function Suggestion(lampId, date, dimmLevel){
        ...
        console.log("SUGGESTION OBJECT: "+dimmLevel);
        ...
        this.scheduledSuggestion = schedule.scheduleJob(date, (function(){
            console.log("THE SCHEDULED TASK EXECUTED: "+this.dimmLevel);
